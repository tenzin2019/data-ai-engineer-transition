...
# This script creates Azure PostgreSQL database and configures the application
...
DB_SERVER_NAME="postgres-document-analysis"
...
echo -e "${YELLOW}🗄️  Step 1: Creating Azure PostgreSQL database...${NC}"
...
if az postgres flexible-server db show --name $DB_NAME --server-name $DB_SERVER_NAME --resource-group $RESOURCE_GROUP &> /dev/null; then
...
DB_HOST=$(az postgres flexible-server show --name $DB_SERVER_NAME --resource-group $RESOURCE_GROUP --query fullyQualifiedDomainName --output tsv)
...
echo "✅ Azure PostgreSQL database created"
